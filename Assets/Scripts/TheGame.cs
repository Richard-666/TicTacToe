...
public class TheGame : MonoBehaviour
{
    ...
    private int turns; // 游戏回合数
    private int dogfall; // 平局数
    private bool unlock; // 游戏结束锁定棋盘
    ...
    void OnGUI()
    {
        ...
        GUI.Box(new Rect(0, 0, Screen.width, Screen.height), ""); // 全屏
        ...
        GUI.BeginGroup(new Rect(Screen.width * 0.5f - 300, Screen.height * 0.5f - 200, gamePadWidth, gamePadHeight));
            ...
            GUI.Label(new Rect (2 * padding + 3 * chessSideLength, padding, 150, 100), "TicTacToe"); // 游戏Title
    ...
    private void setGameModeButton(){
        if(gameMode == SINGLE){
            if(GUI.Button(new Rect (90,30,40,40), singleModeLogo)){
                resetChessboard();
                gameMode = DOUBLE;
    ...
    private void setResetButton(){
        if(GUI.Button(new Rect (0,30,40,40), restartLogo)){
            resetChessboard();
        }
    }
    ...
    private async void robotAction(){
        int chessStyleThisTurn = getChessStyleThisTurn();
